...
import { setItem, getAllItems } from '../../utils/indexedDB';
...
const Signup: React.FC<SignupProps> = ({ zindex, setSignup, setShowModal, setAnimate, animate }) => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (email !== '' && password !== '' && username !== '') {
      ...
      const users: User[] = await getAllItems();
      ...
      alert('Please fill complete registration form!');
    }
  };
    ...
    <>
      <div tabIndex={-1} className={`max-h-full ${zindex} w-full overflow-x-hidden overflow-y-auto md:inset-0 ${animate ? 'animate-in' : 'animate-out'}`}>
        <div className="relative w-full max-w-lg max-h-full mx-auto flex justify-center">
          <div
            className={`gradient-border w-[463px] ${
              zindex ? 'fixed top-20' : 'relative'
            } bg-[#27292D] shadow px-6 py-10 flex flex-col justify-center items-center gap-[45px]`}
          >
            <div className="flex flex-col justify-center items-center gap-2">
              <span className="font-inter font-medium text-sm text-[#6B6C70]">SIGN UP</span>
              <span className="font-inter font-semibold text-lg text-[#FFF]">Create an account to continue</span>
            </div>
            <form onSubmit={handleSubmit} className="flex flex-col gap-5 w-full">
              <div className="flex flex-col gap-4 w-full">
                <div className="flex flex-col w-full">
                  <label htmlFor="email" className="text-[#C5C7CA] text-sm font-lato font-medium mb-[10px]">
                    Email address
                  </label>
                  <input
                    type="email"
                    ...
                    value={email}
                    className="bg-transparent border-[1.5px] border-[#35373B] rounded-[4px] p-3 text-[#7F8084] text-base leading-[19.36px] font-inter font-normal placeholder:text-[#7F8084] placeholder:text-base placeholder:leading-[19.36px] placeholder:font-inter placeholder:font-normal focus:outline-none"
                  />
                </div>
                <div className="flex flex-col">
                  <label htmlFor="username" className="text-[#C5C7CA] text-sm font-lato font-medium mb-[10px]">
                    Username
                  </label>
                  <input
                    type="username"
                ...
                <div className="relative flex flex-col">
                  <label htmlFor="password" className="text-[#C5C7CA] text-sm font-lato font-medium mb-[10px]">
                    Password
                  </label>
                  <input
              ...
              <div className='flex flex-col justify-center items-start gap-3 w-full'>
                ...
                <div className='text-[#7F8084] text-sm leading-[16.94px] font-inter font-medium'>Already have an account?<span className='text-[#C5C7CA] cursor-pointer' onClick={() => setSignup(false)}> Login →</span></div>
              </div>
            </form>
          </div>
